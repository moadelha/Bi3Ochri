...
def submit_number(request):
    ...
    if request.method == 'POST':
        try:
            ...
            ph_number = int(request.POST.get('phone_number', 0))
            ph_name = request.POST.get('phone_name', '')
            otp_car_id = request.POST.get('car_id', '')

            if not ph_number or not otp_car_id:
            ...
            if Client.objects.filter(phone_number=ph_number).exists():
                ...
                # Use reverse to generate URLs safely
                rel_url = reverse('listing_detail', args=[otp_car_id])  # 'listing_detail' should be your url pattern name
                return redirect(f'{rel_url}#get_details')

            else:
                # Generate OTP and store in session
                request.session['session_otp'] = generate_otp_message(request)
                re_url = reverse('listing_detail', args=[otp_car_id])
                return redirect(f'{re_url}#submit_otp')
